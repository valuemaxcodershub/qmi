...
@section('content')
                ...
                <div class="modal-body pt-0 ">
                    <div class="product-table-wrap">
                        <div class="table-responsive">
                            <table class="table __table text-capitalize text-start table-align-middle">
                                <thead class="mb-3">
                                    <tr>
    ...
    <div class="container rtl">
        ...
        <h2 class="headerTitle text-center my-3">{{ translate('my_Order')}}</h2>

        <div class="btn--primary">
            <div class="container py-2 py-lg-3">

                <div class="d-flex justify-content-between order-lg-1 pr-lg-4 text-center text-lg-left">
                    @if(auth('customer')->check())
                        <h4 class="text-light mb-0">{{ translate('order_ID')}} :
                            <span class="h4 font-weight-normal text-light cursor-pointer" onclick="location.href='{{ route('account-order-details', ['id'=>$orderDetails->id]) }}'">
                                {{$orderDetails['id']}}
                            </span>
                        </h4>
                    @else
                        <h4 class="text-light mb-0">{{ translate('order_ID')}} :
                            <span class="h4 font-weight-normal text-light cursor-pointer" onclick="location.href='{{route('customer.auth.login')}}'">
                                {{$orderDetails['id']}}
                            </span>
                        </h4>
                    @endif
                    @if ($order_verification_status && $orderDetails->order_type == "default_type")
                        <h4 class="text-light mb-0">{{ translate('verification_code')}} :
                            <span class="h4 font-weight-normal text-light cursor-pointer">
    ...
    <div class="container mb-md-3  rtl">
        ...
        <div class="row __bg-e2f0ff w-100 m-0">
            <div class="col-sm-{{ $orderDetails['expected_delivery_date'] ? '3':'4' }}">
                <div class="pt-2 pb-2 text-center rounded-lg">
                    <span class="font-weight-medium text-dark mr-2">{{ translate('order_Status')}}:</span><br>
            ...
            <div class="col-sm-{{ $orderDetails['expected_delivery_date'] ? '3':'4' }}">
                <div class="pt-2 pb-2 text-center rounded-lg">
                    <span class="font-weight-medium text-dark mr-2">{{ translate('payment_status')}}:</span> <br>
                    <span class="text-uppercase">{{ translate($orderDetails['payment_status'])}}</span>
                </div>
            </div>
            <div class="col-sm-{{ $orderDetails['expected_delivery_date'] ? '3':'4' }}">
                <div class="pt-2 pb-2 text-center rounded-lg">
                    <span class="font-weight-medium text-dark mr-2"> {{ translate('Order_Created_At')}}: </span> <br>
            ...
            <div class="col-sm-3">
                <div class="pt-2 pb-2 text-center rounded-lg">
                    <span class="font-weight-medium text-dark mr-2"> {{ translate('estimated_Delivary_Date')}}: </span> <br>
                    <span class="text-uppercase font-semibold" style="color: {{$web_config['primary_color']}}">{{Carbon\Carbon::createFromFormat('Y-m-d',$orderDetails['expected_delivery_date'])->format('Y-m-d')}}</span>
                </div>
            </div>
            @endif
        </div>
                            ...
                            <div class="nav-link">
                                <div class="align-items-center">
                                    ...
                                    <div class="media-body text-center">
                                        <div class="media-tab-subtitle text-muted font-size-xs mb-1">{{ translate('first_step')}}</div>
                                        <h6 class="media-tab-title text-nowrap mb-0">{{ translate('order_placed')}}</h6>
                                    </div>
                                </div>
                            </div>
                        </li>
                        ...
                        <li class="nav-item ">
                            <div class="nav-link ">
                                <div class="align-items-center">
                                    ...
                                    <div class="media-body text-center">
                                        <div class="media-tab-subtitle text-muted font-size-xs mb-1">{{ translate('second_step')}}</div>
                                        <h6 class="media-tab-title text-nowrap mb-0">{{ translate('packaging_order')}}</h6>
                                    </div>
                                </div>
                            </div>
                        </li>
                        ...
                        <li class="nav-item">
                            <div class="nav-link  ">
                                <div class="align-items-center">
                                    ...
                                    <div class="media-body text-center">
                                        <div class="media-tab-subtitle text-muted font-size-xs mb-1">{{ translate('third_step')}}</div>
                                        <h6 class="media-tab-title text-nowrap mb-0">{{ translate('preparing_Shipment')}}</h6>
                                    </div>
                                </div>
                            </div>
                        </li>
                        ...
                        <li class="nav-item">
                            <div class="nav-link ">
                                <div class="align-items-center">
                                    ...
                                    <div class="media-body text-center">
                                        <div class="media-tab-subtitle text-muted font-size-xs mb-1">{{ translate('fourth_step')}}</div>
                                        <h6 class="media-tab-title text-nowrap mb-0">{{ translate('order_Shipped')}}</h6>
                                    </div>
                                </div>
                            </div>
                        </li>
                    @elseif($orderDetails['order_status']=='returned')
                        <li class="nav-item">
                            <div class="nav-link text-center">
                                <h1 class="text-warning">{{ translate('product_Successfully_Returned')}}</h1>
                            </div>
                        </li>
                    @else
                        <li class="nav-item">
                            <div class="nav-link text-center">
                                <h1 class="text-danger">{{ translate("sorry_we_can_not_complete_your_order")}}</h1>
                            </div>
                        </li>
                    @endif
        ...
        <div class="d-sm-flex flex-wrap justify-content-between align-items-center text-center pt-3">
            ...
            <a class="btn btn--primary btn-sm mt-2 mb-2" href="#order-details" data-toggle="modal">{{ translate('view_Order_Details')}}</a>
        </div>
    </div>
