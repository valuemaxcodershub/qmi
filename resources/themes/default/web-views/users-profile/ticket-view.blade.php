...
@section('content')
    ...
    <div class="container rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        <h2 class="m-0 headerTitle text-center py-3">{{translate('support_Ticket_Answer')}}</h2>
    </div>
    <!-- Page Content-->
                    ...
                    <div class="table-responsive">
                        <table class="table __table mb-0">
                            <thead class="thead-light">
                                <tr>
                                    <th>{{translate('date_Submitted')}}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($ticket['status']=='open')
                                            <span class="badge btn btn-secondary">{{translate($ticket['status'])}}</span>
                                        @else
                                            <span class="badge btn btn-secondary">{{translate($ticket['status'])}}</span>
                                        @endif
                                    </td>
                    ...
                    <div class="__media-wrapper card-body">
                        ...
                        @foreach($ticket->conversations as $conversation)
                            ...
                            @if($conversation['admin_message'] == null)
                                <div class="media __incoming-msg">
                                    <img class="rounded-circle __img-40" height="40" width="40"
                                        onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
                                        src="{{asset('storage/app/public/profile')}}/{{auth('customer')->user()->image}}"
                                        alt="{{auth('customer')->user()->f_name}}"/>
                                    <div class="media-body">
                        ...
                        <form class="needs-validation" href="{{route('support-ticket.comment',[$ticket['id']])}}"
                            ...
                            <div class="form-group">
                                <textarea class="form-control" name="comment" rows="8"
                                        placeholder="{{translate('write_your_message_here')}}..." required></textarea>
                                <div class="invalid-tooltip">{{translate('please_write_the_message')}}!</div>
                            </div>
                            <div class="d-flex flex-wrap justify-content-end align-items-center gap-8">
                                <div>
                                    <a href="{{route('support-ticket.close',[$ticket['id']])}}" class="btn btn-secondary text-white">{{translate('close')}}</a>
                                </div>
                                <button class="btn btn--primary my-2" type="submit">{{translate('submit_message')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
