...
@php
    ...
    if (auth('customer')->check()) {
        $loggedUser = auth('customer')->user();
        if (strlen($loggedUser->f_name) > 15 && strpos($loggedUser->f_name, ' ') !== false) {
            // Explode the name by space and take the first part
            $clientNameArray = explode(' ', $loggedUser->f_name);
            $clientName = $clientNameArray[0];
        } else {
            // Otherwise, take the first 15 characters of the name
            $clientName = substr($loggedUser->f_name, 0, 15);
        }
    }
    ...
    <div class="header-top py-2">
        <div class="container">
            <div class="  d-flex align-items-center flex-wrap justify-content-between gap-2">
                <a href="tel:+{{ $web_config['phone']->value }}" class="d-flex gap-2 align-items-center text-white">
                ...
                <ul class="nav justify-content-center justify-content-sm-end align-items-center gap-4">
                            ...
                            @if ($web_config['currency_model'] == 'multi_currency')
                                ...
                                <ul class="dropdown-menu" style="--bs-dropdown-min-width: 10rem">
                                    @foreach ($web_config['currencies'] as $key => $currency)
                                        <li onclick="currency_change('{{ $currency['code'] }}')"><a
                                                href="javascript:">{{ $currency->name }}</a></li>
                                    @endforeach
                    ...
                    <li class="d-none d-xl-block">
                        <a href="{{ route('boost-product') }}" class="d-flex text-white">
                            <div class="fz-16">Boost Products</div>
                        </a>
                    </li>

                    <li class="d-none d-xl-block">
                        <a href="{{ route('trade-assurance') }}" class="d-flex text-white">
                            <div class="fz-16" style="border-left:1px solid #fff; padding-left: 20px">Trade Assurance</div>
                        </a>
                    </li>

                    <li class="d-none d-xl-block">
                        <a href="{{ route('premium-services') }}" class="d-flex text-white">
        ...
        <div class="container">
                ...
                <div>
                    <div class="aside-body" data-trigger="scrollbar">
                        ...
                        <ul class="main-nav nav">
                                ...
                                <ul class="sub_menu">
                                    @foreach ($categories as $key => $category)
                                        <li>
                                            <a href="javascript:">
                                                <span
                                                    onclick="location.href='{{ route('products', ['id' => $category['id'], 'data_from' => 'category', 'page' => 1]) }}'">{{ $category['name'] }}</span>
                                            </a>
                            ...
                            <li>
                                <a href="{{ route('boost-product') }}">Boost Products</a>
                            </li>
                            <li>
                                <a href="{{ route('trade-assurance') }}">Trade Assurance</a>
                            </li>
                            <li>
                                <a href="{{ route('premium-services') }}">Premium Services</a>
                            </li>
                    ...
                    <div class="d-flex align-items-center gap-2 justify-content-between p-4">
                        ...
                        <div class="theme-bar p-1">
                            <button class="light_button active">
                                <img src="{{ theme_asset('assets/img/svg/light.svg') }}" alt=""
                                    class="svg">
                            </button>
                            <button class="dark_button">
                                <img src="{{ theme_asset('assets/img/svg/dark.svg') }}" alt=""
                                    class="svg">
                            </button>
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-between gap-3 align-items-center position-relative">
                                    ...
                                    <div class="sub-menu megamenu p-3" style="--bs-dropdown-min-width: max-content">
                                        <div class="d-flex gap-5">
                                            <div class="column-2 row-gap-3">
                                                @foreach ($web_config['shops'] as $shop)
                                                    {{-- {{ $shop->seller->business_shortcode }} --}}
                                                    <a href="{{ route('seller-shop', $shop->seller->business_shortcode) }}"
                ...
                <ul class="list-unstyled d-flex d-lg-none  list-separator mb-0 pe-2">
                    @if (auth('customer')->check())
                        <li class="login-register d-flex align-items-center gap-4">
                            <div class="profile-dropdown d-none d-lg-flex">
                                <button type="button"
                                    class="border-0 bg-transparent d-flex gap-2 align-items-center dropdown-toggle text-dark p-0 user"
                                    data-bs-toggle="dropdown" aria-expanded="false">
