...
@push('css_or_js')
    @if ($shop['id'] != 0)
        ...
        <meta property="og:url" content="{{ route('seller-shop', $shop['seller']['business_shortcode']) }}">
    @else
    ...
    @if ($shop['id'] != 0)
        ...
        <meta property="twitter:title" content="{{ route('seller-shop', $shop['seller']['business_shortcode']) }}" />
        <meta property="twitter:url" content="{{ route('seller-shop', $shop['seller']['business_shortcode']) }}">
    @else
...
@section('content')
    ...
    <main class="main-content d-flex flex-column gap-3 py-3">
        <div class="container">
                        ...
                        @if ($shop['id'] != 0)
                            <div class="media gap-3">
                                ...
                                <div class="media-body d-flex flex-column gap-2">
                                    ...
                                    <div class="d-flex gap-2 align-items-center">
                                        <span class="star-rating text-gold fs-12">
                                            @for ($i = 1; $i <= 5; $i++)
                                                ...
                                                @elseif ($avg_rating != 0 && $i <= (int) $avg_rating + 1 && $avg_rating >= (int) $avg_rating + 0.3)
                                    ...
                                    <h6><span class="follower_count">{{ $followers }}</span>
                                        {{ translate('Followers') }}</h6>
                                ...
                                <div class="media-body d-flex flex-column gap-2">
                                    ...
                                    @if ($minimum_order_amount_status == 1 && $minimum_order_amount_by_seller == 1)
                                        ...
                                        @if ($shop['id'] == 0)
                                            ...
                                            <span
                                                class="text-sm-nowrap">{{ \App\CPU\Helpers::currency_converter($minimum_order_amount) }}
                                                {{ translate('minimum_order_amount') }}</span>
                                        @else
                                            <span
                                                class="text-sm-nowrap">{{ \App\CPU\Helpers::currency_converter($shop->seller->minimum_order_amount) }}
                                                {{ translate('minimum_order_amount') }}</span>
                                        @endif
                                    @endif
                                </div>
                            ...
                            <div class="card flex-grow-1">
                                <div class="card-body grid-center">
                                    <div class="text-center">
                                        ...
                                        <p class="text-muted">{{ translate('Total_Active_Products') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            {{-- @if ($shop['id'] != 0 && $shop->bottom_banner)
                <div class="">
                    <img src="{{ asset('storage/app/public/shop/banner') }}/{{ $shop->bottom_banner }}"
                        class="dark-support rounded img-fit"
            ...
            @elseif($shop['id'] == 0)
                ...
                @if ($bottom_banner)
                    <div class="">
                        <img src="{{ asset('storage/app/public/shop') }}/{{ $bottom_banner ?? '' }}"
                            class="dark-support rounded img-fit"
                                            ...
                                            <div class="swiper-wrapper">
                                                @foreach ($featured_products as $product)
                                                    <div class="swiper-slide mx-w300">
                                                        <!-- Single Product -->
                                                        @include(
                                                            'theme-views.partials._product-large-card',
                                                            ['product' => $product]
                                                        )
                                                    </div>
                                                @endforeach
                            ...
                            <div class="">
                                <div class="d-flex flex-wrap flex-lg-nowrap gap-2">
                                    <div class="search-box">
                                        <form method="get"
                                            action="{{ route('seller-shop', $shop['seller']['business_shortcode']) }}">
                                            <div class="d-flex">
                                            ...
                                            <div class="dropdown">
                                                <button type="button"
                                                    ...
                                                    $data['data_from'] == 'most-favorite'
                                                        ? str_replace(['-', '_', '/'], ' ', translate($data['data_from']))
                                                        : translate('Choose_Option') }}
                                                </button>
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li class="{{ $data['data_from'] == 'latest' ? 'selected' : '' }}">
                                                        {{-- {{ route('seller-shop', $shop['seller']['business_shortcode']) }} --}}
                                                        <a class="d-flex"
                                                            href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'data_from' => 'latest', 'page' => 1]) }}">
                                                            {{ translate('Latest_Products') }}
                                                        </a>
                                                    </li>
                                                    <li
                                                        class="{{ $data['data_from'] == 'best-selling' ? 'selected' : '' }}">
                                                        <a class="d-flex"
                                                            href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'data_from' => 'best-selling', 'page' => 1]) }}">
                                                            {{ translate('Best_Selling') }}
                                                        </a>
                                                    </li>
                                                    <li class="{{ $data['data_from'] == 'top-rated' ? 'selected' : '' }}">
                                                        <a class="d-flex"
                                                            href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'data_from' => 'top-rated', 'page' => 1]) }}">
                                                            {{ translate('Top_Rated') }}
                                                        </a>
                                                    </li>
                                                    <li
                                                        class="{{ $data['data_from'] == 'most-favorite' ? 'selected' : '' }}">
                                                        <a class="d-flex"
                                                            href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'data_from' => 'most-favorite', 'page' => 1]) }}">
                                                            {{ translate('Most_Favorite') }}
                                                        </a>
                                                    </li>
                                                    @if ($web_config['featured_deals'])
                                                        <li
                                                            class="{{ $data['data_from'] == 'featured_deal' ? 'selected' : '' }}">
                                                            <a class="d-flex" href="">
                                                                <a class="d-flex"
                                                                    href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'data_from' => 'featured_deal', 'page' => 1]) }}">
                        ...
                        <div class="card-body d-flex flex-column gap-4">
                                        ...
                                        @foreach ($categories as $category)
                                            <li>
                                                <div class="d-flex justify-content-between">
                                                    <a
                                                        href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'category_id' => $category['id']]) }}">{{ $category['name'] }}</a>
                                                    ...
                                                    <ul class="sub_menu">
                                                        @foreach ($category->childes as $child)
                                                            <li>
                                                                <div class="d-flex justify-content-between">
                                                                    <a
                                                                        href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'category_id' => $child['id']]) }}">{{ $child['name'] }}</a>
                                                                        ...
                                                                        @foreach ($child->childes as $ch)
                                                                            <li>
                                                                                <label class="custom-checkbox">
                                                                                    <a
                                                                                        href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'category_id' => $ch['id']]) }}">{{ $ch['name'] }}</a>
                                                                                </label>
                                                                            </li>
                                                                        @endforeach
                                                                    </ul>
                                            ...
                                            @foreach ($brands as $brand)
                                                <li>
                                                    <div class="flex-between-gap-3 align-items-center">
                                                        <label class="custom-checkbox">
                                                            <a
                                                                href="{{ route('seller-shop', [$shop['seller']['business_shortcode'], 'brand_id' => $brand->id]) }}">{{ $brand['name'] }}</a>
                                                        </label>
                            ...
                            <div>
                                ...
                                <section class="range-slider">
                                    ...
                                    <input name="rangeOne" value="0" min="0" max="10000" step="1"
                                        type="range" id="price_rangeMin">
                                    <input name="rangeTwo" value="5000" min="0" max="10000" step="1"
                                        type="range" id="price_rangeMax">
                                </section>
                            </div>
                        </div>
                    </div>
    ...
    <span id="filter_url" data-url="{{ route('seller-shop', $shop['seller']['business_shortcode']) }}"></span>
