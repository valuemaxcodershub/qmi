                                ...
                                <div class="form-group">
                                    <div class="d-flex justify-content-between align-items-center gap-10 form-control">
                                        <span class="title-color">
                                            ...
                                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                                data-placement="top"
                                                title="{{ translate('offline_Payment_allows_customers_to_use_external_payment_methods._They_must_share_payment_details_with_the_seller_afterward._Admin_can_set_whether_customers_can_make_offline_payments_by_enabling/disabling_this_button.
                                                                                                                                                                                                                                                                                                                                                                                        ') }}">
                                                <img width="16"
                                                    src="{{ asset('/public/assets/back-end/img/info-circle.svg') }}"
                                                    alt="">
                    ...
                    <div class="card">
                        <form action="{{ route('admin.business-settings.payment-method.addon-payment-set') }}"
                            ...
                            <div class="card-body">
                                ...
                                @if (strtolower($payment->key_name) == 'flutterwave')
                                    <div class="alert alert-info">
                                        <strong>INFO: </strong> This payment processor is used in paying out to Buyers or
                                        Sellers on PAVI
                                    </div>
                                @endif
        ...
        function paymentMethodStatusModal(e, toggle_id, image, on_title, off_title, on_message, off_message) {
            ...
            if ($('#' + toggle_id).is(':checked')) {
                ...
                $('.toggle-modal-img-box .status-icon').attr('src',
                    '{{ asset('/public/assets/back-end/img/modal/status-green.png') }}');
            } else {
                $('#toggle-status-title').empty().append(off_title);
                $('#toggle-status-message').empty().append(off_message);
                $('#toggle-status-ok-button').attr('toggle-ok-button', toggle_id);
                $('.toggle-modal-img-box .status-icon').attr('src',
                    '{{ asset('/public/assets/back-end/img/modal/status-warning.png') }}');
            }
