    ...
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.business-settings.update-info') }}" method="POST"
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h5 class="mb-0 text-capitalize d-flex gap-1">
                        <i class="tio-briefcase"></i> Boost Product Settings
                    </h5>
                </div>
                <div class="card-body">
                    <div class="row align-items-end">
                        @php($config=\App\CPU\Helpers::get_business_settings('product_boosting_setting'))
                        <div class="col-sm-6 col-lg-4">
                            <div class="form-group">
                                <label class="title-color d-flex" for="currency">Charge Mode</label>
                                <select name="allow_boosting" class="form-control js-select2-custom">
                                    <option value="yes" {{ $config['allow_boosting'] == 'yes' ? "selected" : "" }}>Allow Product Boosting</option>
                                    <option value="no" {{ $config['allow_boosting'] == 'no' ? "selected" : "" }}>Disable Product Boosting</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-4">
                            <div class="form-group">
                                <label class="title-color d-flex" for="currency">Charge Mode</label>
                                <select name="charging_mode" class="form-control js-select2-custom">
                                    <option value="seller_type" {{ $config['charging_mode'] == 'seller_type' ? "selected" : "" }}>Charge Based on Seller Type Boosting Mode</option>
                                    <option value="general_mode" {{ $config['charging_mode'] == 'general_mode' ? "selected" : "" }}>Charge Based on General Boosting Mode</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-6 col-lg-4">
                            <div class="form-group">
                                <label class="title-color d-flex" for="currency">General Boosting Charges</label>
                                <input name="boosting_fee" class="form-control" value="{{ $config['boosting_fee'] }}">
                            </div>
                        </div>
