...
@section('title', 'Credit Loyalty Wallet')
        ...
        <div class="card">
            <div class="p-3">
                <div class="card-header row justify-content-between align-items-center">
                    <h5 class="ml-3">
                        Credit Loyalty Wallet
                    </h5>
                </div>
            </div>
            <form action="{{ route('admin.competition.credit-loyaltywallet') }}" method="post">
                @csrf @method('PUT')
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="input-label" for="minimum_sales_amount">Enter Seller's Email</label>
                                <input type="email" name="email_address" class="form-control email_address" value="{{old('email_address')}}" 
                                    placeholder="{{translate('enter_seller_email_address')}}" required>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="input-label" for="minimum_sales_amount">Seller's Name</label>
                                <input type="email" class="form-control seller_name" disabled />
                            </div>
                        </div>
                ...
                <div class="card-footer">
                    <button class="btn btn-primary" type="submit" disabled>Credit Wallet</button>
                </div>
            </form>         
        </div>
    </div>
...
@push('script_2')
    ...
    <script>
        $(".email_address").on("keyup", function() {
            let email = $(this).val();
            let responseField = $(".seller_name");
            let spanSearchField = $(".span_search");
            ...
            if (validateEmail(email) && email != "") {
                $.ajax({
                    url: "{{ route('search-seller') }}",  // Use double quotes and interpolation for the route
                    type: 'GET',
                    data: {
                        email: email
                    },
                    beforeSend: function() {
                        spanSearchField.html("<em class='text-danger'>Searching for seller...</em>");
                    },
                    success: function(result) {
                        console.log(result);
                        spanSearchField.html("");
                        responseField.val(result.data.fullname);
                    ...
                    error: function(xhr, status, error) {
                        ...
                        let decode = JSON.parse(xhr.responseText);
