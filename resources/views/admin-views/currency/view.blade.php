...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <form action="{{ route('admin.currency.store') }}" method="post" style="display: none">
                    @csrf
                    <div class="">
                        <div class="row">
                        ...
                        <table
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="text-center" style="display: none">{{ translate('action') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                            ...
                                            <td>
                                                @if ($default['value'] != $data->id)
                                                    <form action="{{ route('admin.currency.status') }}" method="post"
                                                        id="currency_status{{ $data['id'] }}_form"
                                                        class="currency_status_form">
                                                        @csrf
                                                ...
                                                @else
                                                    <label
                                                        class="badge badge-primary-light">{{ translate('default') }}</label>
                                                @endif
                                            </td>
...
@push('script')
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.status == 1) {
                                toastr.success(
                                    "{{ translate('currency_removed_successfully') }} !");
                                location.reload();
                            } else {
                                toastr.warning(
                                    "{{ translate('this_Currency_cannot_be_removed_due_to_payment_gateway_dependency') }} !"
                                    );
                                location.reload();
                            }
                        }
    ...
    <script>
        function default_currency_delete_alert() {
            toastr.warning(
                '{{ translate('default currency can not be deleted!to delete change the default currency first!') }}');
        }
    </script>
@endpush
