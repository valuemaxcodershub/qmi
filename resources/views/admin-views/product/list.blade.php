                            ...
                            @if($type == 'in_house')
                                <div>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('admin.product.export-excel',['type'=>'in_house'])}}?brand_id={{request('brand_id')}}&category_id={{request('category_id')}}&sub_category_id={{request('sub_category_id')}}&sub_sub_category_id={{request('sub_sub_category_id')}}">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <form action="{{route('admin.product.status-update')}}" method="post" id="product_status{{$p['id']}}_form" class="product_status_form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$p['id']}}">
                                        <input type="hidden" name="active_status" value="{{$p['status']}}">
                                        <label class="switcher mx-auto">
...
@push('script')
    ...
    <script>
        ...
        $('.product_status_form').on('submit', function(event){
            ...
            let activeStatus = $(this).find('input[name="active_status"]').val();
            let productId = $(this).find('input[name="id"]').val();
            let toggleButton = $(this).find('input.switcher_input');
            let initialCheckedState = toggleButton.prop('checked'); // Store the initial state
            ...
            if (activeStatus == 0) { 
                Swal.fire({
                    ...
                    input: 'number',
                    inputLabel: 'Enter the shipping fee:',
                    inputPlaceholder: 'Enter shipping fee',
                    showCancelButton: true,
                    confirmButtonText: 'Confirm',
                    cancelButtonText: 'Cancel',
                    preConfirm: (price) => {
                        if (!price) {
                            Swal.showValidationMessage('Shipping fee is required');
                        } else if (price <= 0) {
                            Swal.showValidationMessage('Shipping fee must be greater than zero');
                        }
                        return price;
                    }
                }).then((result) => {
                    if (result.isConfirmed) {
                        ...
                        // Proceed with the toggle action and set the shipping fee
                        // You can send this price to your server or perform other actions as needed
                        console.log('Confirmed shipping fee:', price);
                        console.log(`Product ID: ${productId}`);
                    } else {
                        // Revert to the original state if cancel is pressed
                        toggleButton.prop('checked', !initialCheckedState);
                    }
                });
                return false;
            }
