...
class Helpers
{
    ...
    public static function isValidAndFutureDate($date, $format = 'Y-m-d')
    {
        try {
            // Create a Carbon instance from the given date and format
            $carbonDate = Carbon::createFromFormat($format, $date);

            // Check for parsing errors
            if ($carbonDate->format($format) !== $date) {
                return false;
            }
...
if (!function_exists('get_business_settings')) {
    ...
    {
        $config = null;
        $check = ['currency_model', 'currency_symbol_position', 'system_default_currency', 'language', 'company_name', 'decimal_point_settings', 'product_brand', 'digital_product', 'company_email', 'product_boosting_setting'];
