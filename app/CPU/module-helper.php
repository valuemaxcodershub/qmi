...
if(!function_exists('add_fund_to_wallet_success')) {
    function add_fund_to_wallet_success($payment_data){
        if (isset($payment_data) && $payment_data['is_paid'] == 1) {
            ...
            $wallet_transaction = CustomerManager::create_wallet_transaction($payment_data['payer_id'], BackEndHelper::currency_to_usd($payment_data['payment_amount']), 'add_fund', 'add_funds_to_wallet',$payment_data['payment_method']);
...
if(!function_exists('get_cart_commission')) {
    function get_cart_commission($payment_data){
        
        return [
            $payment_data, 
            Cart::where(['customer_id' =>  '11', 'is_guest'=>'0'])->groupBy('cart_group_id')->pluck('cart_group_id')->toArray()
        ];
