...
class ProductController extends Controller
{
    ...
    {
        $seller = Seller::with(['sellertype'])->where(['id' => auth('seller')->id()])->first();

        if($seller->seller_type == null) {
            return response()->json([
                'message' => 'Merchant does not have an account type e.g Free or Individual Merchant. Notify Admin'
            ]);
        }
        ...
        if ($request['status'] != 1) {
            ...
            $totalApprovedProduct = Product::where(['user_id' => auth('seller')->id(), 'status' => 1])->count();

            if($totalApprovedProduct == $seller->sellertype->product_limit) {
                return response()->json([
                    "message" => "You have reached maximum active product (".number_format($totalApprovedProduct).") you can have"
                ]);
            }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'delivery_state.required'           => 'Delivery state is required. You can select All State for Nationwide delivery!',
        ]);
        ...
        if ($request->ajax()) {
            ...
            $product->delivery_state = implode(",", $request->delivery_state);
            $product->save();
