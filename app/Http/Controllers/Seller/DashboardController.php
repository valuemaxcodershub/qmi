...
class DashboardController extends Controller
{
    ...
    {
        $sellerType = Seller::with(['sellertype'])->where(['id' => auth('seller')->id()])->first()->sellertype;
        $isFreeSellerType = false;
        if (strtolower($sellerType->name) == 'free') {
            $isFreeSellerType = true;
        }
        if (auth('seller')->user()->business_shortcode == NULL) {
            return redirect()->route('seller.business.business-shortcode');
        }
        ...
        $data['escrow_balance'] = $admin_wallet->escrow_balance ?? 0;
        ...
        $data['loyalty_wallet'] = $admin_wallet->loyalty_wallet ?? 0;
        $data['total_balance'] = ($admin_wallet->total_earning + $admin_wallet->escrow_balance);
        ...
        $isActiveCompetition = Competition::where(['status' => '1'])->where('end_date', '>=', now())->first();
        $activeCompStatus = $isActiveCompetition != null ? true : false;        

        return view('seller-views.system.dashboard', compact('data', 'seller_data', 'commission_data', 'withdrawal_methods', 
                    'sellerType', 'isFreeSellerType', 'activeCompStatus'));
    }
