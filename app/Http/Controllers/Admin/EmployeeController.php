...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('identity_image'))) {
            foreach ($request->identity_image as $img) {
                array_push($identity_images, ImageManager::upload('admin/', 'png', $img));
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($request->file('identity_image'))) {
            ...
            $img_keeper = [];
            foreach ($request->identity_image as $img) {
                array_push($img_keeper, ImageManager::upload('admin/', 'png', $img));
            }
     ...
     */
     ...
     public function export(Request $request){
        $key = $request->has('search') ? explode(' ', $request['search']) : '';
        $employee = Admin::with(['role'])->whereNotIn('id', [1])
                    ...
                    ->get();
        return (new FastExcel($employee))->download('employee_list.xlsx');
    }
}
