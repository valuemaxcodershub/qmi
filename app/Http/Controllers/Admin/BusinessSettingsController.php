...
class BusinessSettingsController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        //product boosting settings...
        BusinessSetting::updateOrInsert(['type' => 'product_boosting_setting'], [
            'value' => json_encode(['allow_boosting' => $request->allow_boosting ?? 'no', 'boosting_fee' => $request->boosting_fee, 'charging_mode' => $request->charging_mode])
        ]);
    ...
    public function updateAppleLogin($service, Request $request)
    {
        ...
        if ($request->hasfile('service_file')) {
            $fileName = ImageManager::upload('apple-login/', 'p8', $request->file('service_file'));
        }
    ...
    public function commission() {
        $sales_commission = BusinessSetting::where(['type' => 'sales_commission'])->first();
        return view('admin-views.business-settings.sales.commission', compact('sales_commission'));
    }

    public function updateCommission(Request $request) {
        $validator = Validator::make($request->all(), [
            'sales_commission' => 'numeric|required'
        ]);
        $data =  $validator->validated();
        $updateComm = BusinessSetting::where(['type' => 'sales_commission'])->update(['value' => $data['sales_commission']]);
        if ($updateComm) {
            Session::flash('success', 'PAVI Sales Commission updated successfully');
            return redirect()->back();
        } else {
            Session::flash('error', 'Error updating sales commission');
            return redirect()->back();
        }
    }
