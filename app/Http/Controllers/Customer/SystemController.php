...
class SystemController extends Controller
{
    ...
    public function choose_shipping_address_other(Request $request){
        ...
        if(isset($shipping['shipping_method_id'])) {
            if ($shipping['contact_person_name'] == null || !isset($shipping['address_type']) || $shipping['address'] == null || $shipping['city'] == null || !isset($shipping['country']) || $shipping['country'] == null || ($is_guest && $shipping['email'] == null)) {
                return response()->json([
                    'errors' => translate('Fill_all_required_fields_of_shipping_address')
                ], 403);
            } elseif ($country_restrict_status && !self::delivery_country_exist_check($shipping['country'])) {
        ...
        if ($stateId === null) {
            return response()->json([
                'errors' => "Please select delivery state"
            ], 403);
        }
        
        $customer_id = auth()->guard('customer')->user()->id;
        $productIds = Cart::where('customer_id', $customer_id)->pluck('product_id')->toArray();
        $getState = States::find($stateId);
        
        // Let's get all state id...
        $allState = States::where(['state_name' => 'All States'])->first();
        $allStateId = "";
        if($allState !== NULL) {
            $allStateId = $allState->id;
        }

        foreach ($productIds as $keyIndex => $id) {
            $product = Product::where('id', $id)->select('delivery_state', 'name')->first();
            $deliveryStateId = $product->delivery_state;

            $deliveryStateArray = explode(',', $deliveryStateId);

            if ($allStateId != "" AND in_array($allStateId, $deliveryStateArray)) {
                continue;
            }
            else if(!in_array($stateId, $deliveryStateArray)) {
                $stateError[$keyIndex] = "Sorrys, we cannot process delivery of this product ({$product->name}) to your state ({$getState->state_name}) at the moment. Please choose another state or remove item from your cart to continue.";
                break;
            }
        }
