...
class HomeController extends Controller
{
    public function __construct(
        ...
        private boosted_products $boosted_products,
    )
    {
    }
    ...
    public function default_theme()
    {
        ...
        $top_sellers = Seller::approved()->with(['shop'])->whereNotNull('business_shortcode')
            ->withCount(['orders'])->orderBy('orders_count', 'DESC')->take(2)->get();
        //end
        ...
        $categories = Category::with('childes.childes')->where(['position' => 0])->priority()->take(10)->get();
    ...
    public function theme_aster()
    {
        ...
        }])->whereNotNull('business_shortcode')
        ->whereHas('product', function ($query) {
            $query->where('added_by', 'seller')->active();
        })
        ...
        // more stores
        $more_seller = $this->seller->approved()->with(['shop', 'product.reviews'])->whereNotNull('business_shortcode')
            ->withCount(['product' => function ($query) {
                $query->active();
            }])
            ->inRandomOrder()
        ...
        // Boosted product option...
        $activeBoostedProducts = boosted_products::where('status', 'active')->with('product')->inRandomOrder()->take(20)->get();
        ...
        $mobileAdsBanner = array_merge($footer_banner, $top_side_banner);
        return view(VIEW_FILE_NAMES['home'],
            compact(
                ...
                'featured_deals', 'flash_deals', 'categories', 'mobileAdsBanner', 'activeBoostedProducts',
            )
        );
    }
    ...
    public function theme_fashion()
    {
        ...
        $top_sellers = $this->seller->approved()->with(['shop','orders','product.reviews'])->whereNotNull('business_shortcode')
    ...
    public function theme_all_purpose(){
        ...
        // start other store
        $more_sellers = $this->seller->approved()->with(['shop', 'product.reviews'])->whereNotNull('business_shortcode')
            ->withCount(['product' => function ($query) {
                $query->active();
            }])
