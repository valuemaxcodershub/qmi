...
class WebController extends Controller
{
    ...
    public function all_sellers(Request $request)
    {
        ...
        // $sellers = Shop::active()->with(['seller.product'])->whereHas('seller', function ($query) {
        //     $query->whereNotNull('business_shortcode');
        // })
        ...
        $sellers = Shop::active()
        ->with(['seller.product', 'seller.sellertype'])
        ->whereHas('seller', function ($query) {
            $query->whereNotNull('business_shortcode');
        })
        ->withCount(['product' => function ($query) {
        ...
        ->orderBy('product_count', 'desc') // Default ordering by product count in descending order
        ->get();
    ...
    public function checkout_details(Request $request)
    {
        ...
        // return $country_restrict_status;
        ...
        $states = States::where('state_name', '!=', 'All States')->orderBy('state_name', 'asc')->get();
        ...
        if (count($cart_group_ids) > 0) {
            return view(VIEW_FILE_NAMES['order_shipping'], compact('physical_product_view', 'zip_codes', 'country_restrict_status',
                'zip_restrict_status', 'countries', 'states', 'billing_input_by_customer','default_location','shipping_addresses','billing_addresses'));
    ...
    public function checkout_complete_wallet(Request $request = null)
    {
        ...
        if( $cartTotal > $user->wallet_balance)
        {
            ...
            OrderManager::split_seller_admin_fund($unique_id);
    ...
    public function search_seller(Request $request) {
        ...
        if (!$seller) {
            return response()->json(['message' => 'Seller not found', 'status' => false], 400);
        }
        
        if (strtolower($seller->status) != 'approved') {
            return response()->json(['message' => 'Seller is yet to be verified', 'status' => false], 400);
        }
        $seller->fullname = $seller->f_name . ' '. $seller->l_name;
        return response()->json(['message' => 'Seller found!', 'status' => true, 'data' => $seller ], 200);
    }
