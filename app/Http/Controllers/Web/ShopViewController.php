...
class ShopViewController extends Controller
{
    ...
    public function getSellerWithShopName($name) {

        $seller = Seller::where('business_shortcode', $name)->first();
        if($seller == NULL) {
            Session::flash('error', 'Store could not be found');
            return redirect()->route('home');
        }
        $request = new Request();
        return self::seller_shop($request, $seller->id);
    }

    //for seller Shop
    public function seller_shop($request, $id)
    {
    ...
    public function theme_aster($request, $id){
        ...
        if($id == 0){
            ...
            $total_product_count = $query->count();
            $products_for_review = $query->withCount('reviews')->count();
            ...
            $query = Product::active()->where(['added_by' => 'seller', 'user_id' => $seller->id]);
