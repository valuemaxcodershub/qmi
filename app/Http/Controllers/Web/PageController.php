...
class PageController extends Controller
{
    ...
    public function tradeAssurance() {
        $trade_assurance = BusinessSetting::where('type', 'trade-assurance')->first();
        if(!$trade_assurance){
            return back();
        }
        $trade_assurance = json_decode($trade_assurance->value)->content;
        $page_title_banner = $this->business_settings->where('type', 'banner_cancellation_refund_return')->whereJsonContains('value', ['status' => '1'])->first('value');
        return view(VIEW_FILE_NAMES['trade_assurance'], compact('trade_assurance','page_title_banner'));
    }

    public function boostProduct() {
        $page_title_banner = $this->business_settings->where('type', 'banner_cancellation_refund_return')->whereJsonContains('value', ['status' => '1'])->first('value');
        return view(VIEW_FILE_NAMES['boost_product'], compact('page_title_banner'));
    }

    public function cancellation_refund_return() {
        $cancellation_policy = BusinessSetting::where('type', 'cancellation-refund-return')->first();
        if(!$cancellation_policy){
            return back();
        }
        $cancellation_policy = json_decode($cancellation_policy->value)->content;
        $page_title_banner = $this->business_settings->where('type', 'banner_cancellation_refund_return')->whereJsonContains('value', ['status' => '1'])->first('value');
        return view(VIEW_FILE_NAMES['cancellation_refund_return'], compact('cancellation_policy','page_title_banner'));
    }

    public function premiumService() {
        $premium_service = BusinessSetting::where('type', 'premium-services')->first();
        if(!$premium_service){
            return back();
        }
        $premium_service = json_decode($premium_service->value)->content;
        $page_title_banner = $this->business_settings->where('type', 'banner_cancellation_refund_return')->whereJsonContains('value', ['status' => '1'])->first('value');
        return view(VIEW_FILE_NAMES['premium_services'], compact('premium_service','page_title_banner'));
    }
